...
The first step required to run HPAStainR is downloading HPA's normal tissue staining data and their cancer data. The data is available at https://www.proteinatlas.org/about/download, the files required being normal_tissue.tsv.zip and pathology.tsv.zip. However HPAStainR as a function that can download and load the data for you.
```{r, download_data}
library(HPAstainR)
...
The output of HPAStainR is a tibble with multiple columns. The basic columns include the following:
  ...
  - **percent/count_not_detected**: The number or percent of proteins that failed to stain.
  
  - **number of proteins**: The number of proteins tested in a cell type.
  
  - **tested_proteins**: A character string of proteins that were tested in each cell type as not all proteins are tested in every cell type.
  ...
  The **staining score** equation is modeled below, where $t$ is the total number of proteins from the list tested in the cell type, $h$ is the number of proteins with igh staining in the cell type, $m$ is the number of proteins with medium staining in the cell type, $l$ is the number of protein with low staining in the cell type. 
...
You should now be able to query whatever list of proteins you prefer and can easily rank them on whatever column you wish.
